...
class PhonePasswordReset(UserResetPasswordInterface):
    ...
    async def reset_password(self):
        logger.debug(f"Resetting password for phone: {self.identifier}")
        user = await self.user_repository.get_user_by_phone(self.identifier)

        if not user:
            raise UserNotFound("Phone number not registered.")
